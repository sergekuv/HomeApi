...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        //    return StatusCode(200, $"Устройство {device.Id}, имя - {device.Name}, удалено!");
        //}

        [HttpDelete]
        [Route("{nameOrId}")]
        public async Task<IActionResult> DeleteByName([FromRoute] string nameOrId)

        {
            Device device = await _devices.GetDeviceByName(nameOrId);
            if (device == null)
            {
                try
                {
                    Guid guid = new(nameOrId);
                    device = await _devices.GetDeviceById(guid);
                }
                catch (Exception ex)
                {
                    device = null;
                ...
                if (device == null)
                    return StatusCode(400, $"Ошибка: Устройство с именем/Id {nameOrId} не существует.");
            }
            await _devices.DeleteDevice(device);
